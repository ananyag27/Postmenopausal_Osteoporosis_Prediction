...
def clean_age_last_period(data):
    df = data.copy()
    # Filter valid age range (15-65 years)
    df = df.query("RHQ060 >= 15 and RHQ060 <= 65")
    df = common_clean(df)
    return df
...
def move_legend(ax, new_loc, **kws):
    ...
    if old_legend is not None:
        handles, labels = ax.get_legend_handles_labels()  # Correct way to get handles and labels
        title = old_legend.get_title().get_text()
        ax.legend(handles, labels, title=title, loc=new_loc, **kws)
